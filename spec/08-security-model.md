...
| **Compromised root key** | Attacker can sign arbitrary registry content | **Current:** a single Ed25519 root key signs the manifest and revocation list, so compromising that key is sufficient. Recovery requires retiring the compromised `kid` in `root-keys.json` and distributing the updated trust anchor to clients. **Planned:** 3-of-5 threshold signing (FROST over Ed25519) with key shares held offline by independent reviewers, so compromising a single key is insufficient. See the draft [Multi-Signature Ceremony](../docs/multi-sig-ceremony.md). |
...
| **Stale registry at service** | Service has an old copy missing a recent revocation | `expires_at` fields on both manifest and revocation list (currently 12 hours after compilation). Clients MUST reject expired data and fetch fresh copies. The SDK's default cache TTL is 15 minutes. |
